...
var batchRequest = function(params) {
    ...
    var batch = function(req, res, next) {
        ...
        var recurseDependencies = function (reqs) {
            ...
            // debug( 'reqs', reqs );
            // debug( 'requestPromises', requestPromises );

            // @todo somewhere here i need to account for a dependency array
            ...
            if (_.size(requestPromises) >= _.size(reqs)) {
                ...
                _.each(requestPromises, function(rp, key) {
                    // debug( 'rp', rp ); // the request's promise
                    // debug( 'key', key ); // the request's key - ex. "uploadFile", "emailLink"

                    var dependentKey = null;
                    var dependent = _.find(reqs, function(request, dKey) {
                        debug( 'dKey', dKey ); // the request's key - ex. "uploadFile", "emailLink"
                        debug( 'request', request );
                        dependentKey = dKey;

                        var isDependency = false;

                        // debug( 'requestPromises[dKey]', requestPromises[dKey] );
                        // debug( 'request.dependency', request.dependency );
                    ...
                    debug( 'dependent', dependent );
                    // dependent = dependent[0];

                    debug( 'rp', rp ); // the request's promise
                    debug( 'key', key ); // the request's key - ex. "uploadFile", "emailLink"
                    debug( 'dependentKey', dependentKey );
                    debug( 'requestPromises', requestPromises );
                    ...
                    if (dependent) {
                        ...
                        debug( 'dependencyPromises', dependencyPromises );

                        requestPromises[ dependentKey ] = Promise.all( dependencyPromises ).then( function( dependencyResponses ) {
                            debug( 'dependencyResponses', dependencyResponses );
                            // Looking good here.  dependencyResponses is an array of responses from the dependencyPromises

                            // This could break if the data that's being swapped in contains quotes or other JSON-breaking characters
                            // @todo Check out es6-template-strings/resolve / customTags
                            var jsonRequest = JSON.stringify( dependent );

                            var compiled = compile( jsonRequest );

                            // The documentation suggests using the spread operator (...), but I'm avoiding it for compatibility with older node versions
                            var escapeSubstitutions = function() {
                                var literals = arguments[ 0 ];
                                var substitutions = Array.prototype.slice.call( arguments, 1 );
                                debug( 'literals', literals );
                                debug( 'substitutions', substitutions );
                            ...
                            var resolved = resolve( compiled, { dependency: dependencyResponses } );
                            debug( 'resolved', resolved );
                            var parsedJsonRequest = escapeSubstitutions.apply( null, resolved );
                            dependent = JSON.parse( parsedJsonRequest );

                            // jsonRequest = template( jsonRequest, { dependency: dependencyResponses } );
                            // dependent = JSON.parse( jsonRequest );

                            debug( 'modified dependent', dependent );
