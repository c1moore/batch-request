                ...
                _.each(requestPromises, function(rp, key) {
                    ...
                    if (dependent) {
                        requestPromises[dependentKey] = rp.then(function( dependencyResponse ) {
                            console.log( dependencyResponse ); // This has the JSON response from the dependency
                            console.log( dependent ); // This has the JSON request from the dependent
                            ...
                            var templateableFields = {
                                form: [
                                    'html', 'subject'
                                ]
                            };

                            // This would be better served by an intelligent (distinguish array from object) recursive function
                            _.each( templateableFields, function( fieldsArray, key ) {
                                _.each( fieldsArray, function( field, index ) {
                                    dependent[ key ][ field ] = template( dependent[ key ][ field ], { dependency: dependencyResponse } );
                                });
                            });

                            console.log( dependent );

                            // dependent.form.html = template( dependent.form.html, { dependency: dependencyResponse } );
